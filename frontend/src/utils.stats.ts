...
export async function buildGamesList(activities: Activity[]): Promise<RecapGameEntry[]> {
...
export async function buildPlatformsList(activities: Activity[]): Promise<RecapPlatformEntry[]> {
    ...
    for (const activity of activities) {
        ...
        if (entry && gamePlaytimeMap) {
            ...
            gamePlaytimeMap.set(activity.game.id, currentGameSeconds + activity.seconds);
        }
    }

    // Determine most played game for each platform
    for (const [platformId, gamePlaytimeMap] of gamePlaytimeByPlatform.entries()) {
        const entry = platformsMap.get(platformId);
        if (entry) {
            ...
            if (mostPlayedGameId !== null) {
                const mostPlayedGame = activities.find(act => act.game.id === mostPlayedGameId)?.game;
